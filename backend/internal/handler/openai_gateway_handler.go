		...
		if err != nil {
			if result != nil && result.ImageCount > 0 {
				...
				if errors.As(err, &failoverErr) {
					...
					// 池模式：同账号重试（全局设置 DisableSameAccountRetry 开启时跳过，直接切换账号）
					if failoverErr.RetryableOnSameAccount && !(h.gatewayService != nil && h.gatewayService.IsDisableSameAccountRetryEnabled(c.Request.Context())) {
		...
		if err != nil {
			if result != nil && result.ImageCount > 0 {
				...
				if errors.As(err, &failoverErr) {
					...
					// 池模式：同账号重试（全局配置 DisableSameAccountRetry 开启时跳过，直接切换账号）
					if failoverErr.RetryableOnSameAccount && !(h.gatewayService != nil && h.gatewayService.IsDisableSameAccountRetryEnabled(c.Request.Context())) {
						retryLimit := account.GetPoolModeRetryCount()
						if sameAccountRetryCount[account.ID] < retryLimit {
							sameAccountRetryCount[account.ID]++
